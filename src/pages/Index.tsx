...
import { MapPin, Plus, CalendarDays } from 'lucide-react'; // Added CalendarDays icon
              ...
              (filteredTasks || []).map((task) => (
                <Card key={task.id} className="shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 rounded-[var(--border-radius)] overflow-hidden">
                  ...
                  <CardContent className="p-4">
                    ...
                    {task.deadline && (
                      <p className="text-[hsl(var(--text-light))] flex items-center mb-2 text-sm">
                        <CalendarDays size={16} className="mr-2" /> Deadline: {new Date(task.deadline).toLocaleDateString()}
                      </p>
                    )}
